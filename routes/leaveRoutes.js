...
const LeaveUpdate = require("../models/LeaveUpdate");
...
function makeTransporter() {
  return nodemailer.createTransport({
    service: "gmail",
    auth: {
      user: "naniy8123@gmail.com"  ,    // set these in .env
...
router.post("/send-email", async (req, res) => {
  const { to = "muthaipallymanasa001@gmail.com", subject, text, employeeName, leaveType, fromDate, toDate, about, leaveId } = req.body;
...
router.get('/status/:id/:action', async (req, res) => {
  ...
  try {
    ...
    // Format: "Approved Sick Leave on April 18, 2025"
    const formattedDate = new Date(updatedLeave.fromDate).toLocaleDateString('en-US', {
      year: 'numeric',
      month: 'long',
      day: 'numeric'
    });
    ...
    const leaveUpdate = new LeaveUpdate({
      ...
      status:       action,
      message       // ✨ New field with formatted message
    });
...
// 4) List the last 10 LeaveUpdate notifications
// 4) List the last 10 LeaveUpdate notifications
...
// 5) Count unseen LeaveUpdate notifications
